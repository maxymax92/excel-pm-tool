...
Every workbook that desktop Excel opens for build, recalculation, semantic preservation, scenario, performance or VBA-refresh work is an exact disposable copy beneath `~/Library/Containers/com.microsoft.Excel/Data/Documents/PMWorkbookAutomation`. Microsoft documents the enclosing Excel `Data/Documents` container for Mac add-in sideloading, and the release environment verifies that Excel can open this private location without an external-file grant prompt. Each operation receives an isolated child directory, and its cleanup path deletes that directory.
